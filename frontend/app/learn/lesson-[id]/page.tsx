  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-accent/5">
      <div className="container-responsive padding-responsive-y">
        ...
        <ChevronRight className="h-4 w-4" />
        <span className="text-foreground">Lesson {lessonData.id}</span>
      </div>
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
              <Badge variant="outline">Lesson {lessonData.id}</Badge>
              <Badge variant="secondary">{lessonData.phase}</Badge>
              <Badge 
                variant={lessonData.difficulty === 'Beginner' ? 'default' : 
                        lessonData.difficulty === 'Intermediate' ? 'secondary' : 'destructive'}
              >
                {lessonData.difficulty}
              </Badge>
            </div>
            <h1 className="text-3xl font-bold tracking-tight mb-2">{lessonData.title}</h1>
            <p className="text-muted-foreground text-lg mb-4">{lessonData.description}</p>
      ...
      <div className="flex items-center justify-between mt-12 pt-8 border-t">
        <div>
          {lessonData.prevLessonId && (
            <Link href={`/learn/lesson-${lessonData.prevLessonId}`}>
        ...
        <div className="flex items-center space-x-4">
          ...
          {lessonData.nextLessonId && (
            <Link href={`/learn/lesson-${lessonData.nextLessonId}`}>
